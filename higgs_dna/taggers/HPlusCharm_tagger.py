...
class HpC_tagger(Tagger):
    """
    H+c Preselection tagger built from tutorial
    """
    def __init__(self, name, options = {}, is_data = None, year = None):
        super(HpC_tagger, self).__init__(name, options, is_data, year)
    ...
    def calculate_selection(self, events):
        ...
        if not self.is_data:
            # Gen Jets
            gen_jet_cut = gen_jet_selections.select_jets(
                    gen_jets = events.GenJet,
                    options = self.options["gen_jets"],
                    clean = {
                        "lead_photons" : {
                            "objects" : events.Diphoton.Photon,
                            "min_dr" : self.options["gen_jets"]["dr_photons"]
                        },
                        #"sublead_photons" : {
                        #    "objects" : events.GenHggHiggsSubleadPhoton,
                        #    "min_dr" : self.options["gen_jets"]["dr_photons"]
                        #},
                    },
                    name = "SelectedGenJet",
            ...
            gen_jets = awkward_utils.add_field(
                    events = events,
                    name = "SelectedGenJet",
                    data = events.GenJet[gen_jet_cut]
            )

            n_gen_jets = awkward.num(gen_jets)
            awkward_utils.add_field(events, "n_gen_jets", n_gen_jets)

            for objects, name in zip([gen_jets], ["gen_jet"]):
                awkward_utils.add_object_fields(
                        events = events,
                        name = name,
                        objects = objects,
                        n_objects = awkward.max(n_gen_jets),
                        dummy_value = DUMMY_VALUE
                )
        ...
        presel_cut = (jet_presel) & z_veto & photon_id_cut
        #presel_cut = (jet_presel) & photon_id_cut
        self.register_cuts(
            names = ["jet presel", "z veto", "photon ID cut", "all"],
            results = [jet_presel, z_veto, photon_id_cut, presel_cut]
        )
